...
export function DispatchSimulationPanel({ overview }: { overview: DispatchOverview | null }) {
  ...
  return <Card className="!rounded-2xl" title="连续智能派单沙盘（全部自动接单）" extra={<Tag color={running ? 'green' : 'default'}>{running ? '演示运行中' : '已暂停'}</Tag>}>
    ...
    <div className="mb-3 rounded-xl border border-indigo-200 bg-indigo-50 px-3 py-2 text-sm text-indigo-900">规则：SOS 永远队首并立即强制调度；技能为硬门槛。普通单按 Top1 专属确认（0–8秒）→ Top3 抢单（8–20秒）→ Top10 扩散抢单（20–35秒）→ 自动兜底（35秒后）推进。评分 = 距离 40% + 路况 25% + 疲劳 10% + 服务评分 25%；服务中的志愿者只显示预告，返程达到 70% 后才重新进入派单队列。</div>
