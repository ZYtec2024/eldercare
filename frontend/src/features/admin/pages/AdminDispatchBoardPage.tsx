...
import { Button, Card, Progress, Select, Table, Tag, Typography } from 'antd'
...
import { fetchAdminDispatchRegions, fetchDispatchOverview } from '@/services/adapters/dispatch-adapter'
...
export default function AdminDispatchBoardPage() {
  ...
  return <div className="space-y-6">
    ...
    <Card className="!rounded-2xl" size="small"><div className="flex flex-wrap items-center gap-3"><b>当前管理区县</b><Select className="min-w-52" value={regionAdcode} options={regions.map((item) => ({ value: item.adcode, label: item.name }))} onChange={setRegionAdcode} /><span className="text-slate-500">候选、订单、SOS 和路线均严格限制在这个区县内。SOS 由系统自动派单，告警中心负责接警与结案。</span></div></Card>
