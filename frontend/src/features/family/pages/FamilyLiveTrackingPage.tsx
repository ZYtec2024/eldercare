...
export default function FamilyLiveTrackingPage() {
  ...
  return <div className="space-y-6"><div className="rounded-3xl bg-gradient-to-r from-violet-700 via-indigo-700 to-blue-700 p-6 text-white shadow-xl"><Typography.Title level={2} className="!mb-1 !text-white">家属实时守护</Typography.Title><Typography.Text className="!text-indigo-100">可一直查看绑定老人位置；志愿者位置仅在已接单/服务中与老人端同步共享，服务结束后立即停止。</Typography.Text></div>
    <Alert showIcon type="info" icon={<LockOutlined />} message="隐私保护已开启" description={tracking?.privacy_message || '可查看老人位置；服务结束后系统停止返回志愿者坐标和路线。'} action={<Button size="small" icon={<SyncOutlined />} loading={loading} onClick={() => load().catch(() => {})}>刷新</Button>} />
